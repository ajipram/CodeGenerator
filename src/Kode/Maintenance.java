...
public class Maintenance extends javax.swing.JInternalFrame {
    ...
    public void tampilKode(){
        ...
        try{
            ...
            Connection con = DriverManager.getConnection(database, user, pass);
            Statement state = con.createStatement();
            String query = "Select * from HMaintenance where No_Bukti like '%"+ thn +"%'order by Kd_Cabang asc, Tanggal desc"; 
            ResultSet rs = state.executeQuery(query);
    ...
    public List<String> BindList(){
        ...
        try{
            ...
            Connection con = DriverManager.getConnection(database, user, pass);
            Statement state = con.createStatement();
            String query = "Select NoBukti from HMaintenance where NoBukti like '%"+ thn +"%'order by TglBukti desc";
            ResultSet rs = state.executeQuery(query);
    ...
    private void btnUbahActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnUbahActionPerformed
        if(txtCustomer.getText().equals("null : null")){
            ...
            try{
                ...
                query.Edit(Kolom(), Data(), "HMaintenance", "No_Bukti", lblNoBukti.getText());
                updateItemProject();
                generateNoBukti();
                int rowCount = tblModel.getRowCount();
                for (int i = rowCount - 1; i >= 0; i--) {
    ...
    public void tampil(String kode){
        try{
            ...
            while(rs.next()){
                ...
                java.util.Date date = new SimpleDateFormat("YYYY-MM-DDTHH:MM:SS").parse(rs.getString(10));
                tglRef.setDate(date);
